verbgroup(V0,V1):- synsetdomaintopicof(V0,V2),synsetdomaintopicof(V1,V2).
verbgroup(V0,V1):- hypernym(V0,V2),derivationallyrelatedform(V2,V3),derivationallyrelatedform(V3,V1).
verbgroup(V0,V1):- hypernym(V0,V2),hypernym(V2,V3),hypernym(V1,V3).
verbgroup(V0,V1):- hypernym(V1,V3),derivationallyrelatedform(V3,V2),derivationallyrelatedform(V2,V0).
verbgroup(V0,V1):- hypernym(V1,V4),hypernym(V0,V3),alsosee(V2,V3).
verbgroup(V0,V1):- hypernym(V0,V2),hypernym(V1,V3),hypernym(V3,V2).
verbgroup(V0,V1):- derivationallyrelatedform(V3,V1),hypernym(V4,V0),alsosee(V2,V4).
verbgroup(V0,V1):- hypernym(V0,V2),derivationallyrelatedform(V1,V4),alsosee(V2,V3).
verbgroup(V0,V1):- hypernym(V1,V4),derivationallyrelatedform(V0,V3),alsosee(V4,V2).
verbgroup(V0,V1):- derivationallyrelatedform(V3,V1),derivationallyrelatedform(V2,V3),hypernym(V2,V0).
verbgroup(V0,V1):- derivationallyrelatedform(V2,V0),derivationallyrelatedform(V3,V2),hypernym(V3,V1).
verbgroup(V0,V1):- derivationallyrelatedform(V1,V4),derivationallyrelatedform(V0,V3),derivationallyrelatedform(V2,V3),derivationallyrelatedform(V2,V4).
verbgroup(V0,V1):- hypernym(V0,V2),hypernym(V1,V4),derivationallyrelatedform(V3,V2),derivationallyrelatedform(V3,V4).
verbgroup(V0,V1):- hypernym(V1,V4),derivationallyrelatedform(V0,V2),derivationallyrelatedform(V2,V3),hypernym(V3,V4).
verbgroup(V0,V1):- derivationallyrelatedform(V1,V4),derivationallyrelatedform(V0,V3),derivationallyrelatedform(V2,V4),hypernym(V2,V1).
verbgroup(V0,V1):- hypernym(V0,V4),derivationallyrelatedform(V1,V2),derivationallyrelatedform(V2,V3),hypernym(V3,V4).
accuracy: 79.8780487804878